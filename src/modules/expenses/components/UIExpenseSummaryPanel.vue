      ...
      <div v-for="(row, idx) in items" :key="idx" class="summary-card glass-card">
        ...
        <div class="amount-row">
          ...
          <div class="text-right">
            ...
            <span class="val">{{ row.expenseCount }}</span>
          </div>
        </div>
...
export default defineComponent({
  ...
  methods: {
    t(key: keyof typeof expensesTexts.English) {
      const texts = expensesTexts[this.selectedLanguage]
      return (texts as Record<string, string>)[key] ?? key
    },
    categoryEntries(row: ExpenseSummaryItem): [string, number][] {
