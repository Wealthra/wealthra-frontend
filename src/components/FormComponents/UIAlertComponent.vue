  ...
  <transition name="error-fade">
    <div v-if="show" class="error-message" :class="{ 'error-success': type === 'success' }">
      <div class="error-icon">
        <img :src="iconSrc" :alt="type === 'error' ? 'error-icon' : 'success-icon'" />
      </div>
...
export default {
  ...
  computed: {
    iconSrc() {
      const iconMap = {
        error: 'src/icons/error.svg',
        success: 'src/icons/check-data.svg',
        warning: 'src/icons/warning.svg',
        info: 'src/icons/info.svg',
      }
      return iconMap[this.type as keyof typeof iconMap]
    },
  },
  watch: {
    show(newVal) {
